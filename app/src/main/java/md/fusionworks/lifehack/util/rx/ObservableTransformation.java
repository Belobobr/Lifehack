...
public class ObservableTransformation {
  ...
  private static <T> Observable.Transformer<Response<T>, T> createApiRequestConfigurationTransformer() {
    return responseObservable -> responseObservable.flatMap(
        tResponse -> Observable.create((Observable.OnSubscribe<Response<T>>) subscriber -> {
        ...
        .retryWhen(new RetryWithDelayIf(Constant.MAX_RETRIES, Constant.RETRY_DELAY_MILLIS,
