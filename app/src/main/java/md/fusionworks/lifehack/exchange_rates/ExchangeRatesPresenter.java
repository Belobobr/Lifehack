...
public class ExchangeRatesPresenter implements ExchangeRatesContract.UserActionsListener {
  ...
  public ExchangeRatesPresenter(ExchangeRatesContract.View exchangeRatesView,
  ...
  private void loadRates(String date) {
    ...
    exchangeRatesRepository.getRates(date)
        ...
        .subscribe(new ObserverAdapter<List<RateModel>>() {
          ...
          @Override public void onError(Throwable e) {
            exchangeRatesView.hideLoading();
            exchangeRatesView.showNotificationToast(Constant.NOTIFICATION_TOAST_ERROR,
                R.string.field_something_gone_wrong);
          }
        });
  }
  ...
  private BestExchangeModel convertBestExchange() {
    ...
    for (BankModel bank : bankList) {
      ...
      if (bank.getId() != 1) {
        ...
        currencyInRateValue = ExchangeRatesUtils.getCurrencyRateValue(bankRateList,
            exchangeRatesView.getCheckedCurrencyInId());
        currencyOutRateValue = ExchangeRatesUtils.getCurrencyRateValue(bankRateList,
            exchangeRatesView.getCheckedCurrencyOutId());
      ...
      exchangeRatesView.setAmountOutText(
          String.format("%.2f", bestExchangeModel.getAmountOutvalue()));
  ...
  @Override public void applyConversion() {
  ...
  @Override public void onRatesDateChanged(Date date) {
  ...
  @Override public void onCurrencyInChanged(int checkedId) {
  ...
  @Override public void onCurrencyOutChanged(int checkedId) {
  ...
  @Override public void onBankSelected(int position, long id) {

    if (position != 0) exchangeRatesView.setBestExchangeBankText("");

    applyConversion();
  }

  @Override public void onWhereToBuyAction() {
  ...
  @Override public void onRetryAction() {

    loadInitialData();
  }

  @Override public void onAmountInChanged(String text) {

    applyConversion();
  }

  @Override public void onShowInfoWindowAction(BranchModel branch) {
